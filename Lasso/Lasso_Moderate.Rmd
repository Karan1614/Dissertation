...
There is a sharp increase in the train error before tending to 402.0133 as the regularization parameter increases. The test error intitally decreases before reaching a minimum value of 292.9697 at lambda = 3.162278e-03. The error then increases gradually as the regularization parameter increases before tending to a value 416.3853 at log10(lambda) = 1. Both the test and train error reach this maximum at this point.
The train and the test error increase as the bias squared increases as the regulation parameter increases while the variance tends to 0. Thus after a point (in this case log10(lambda) = 1) the mean squared error is just the bias squared. This is seen in the next plot.
...
In the graph the bias squared is represeted by a line and the variance is represented by the red line. As we can see from the graph the bias squared and the variance behave in the opposite manner. The Mean Squared Error is a sum of the bias squared and the variance. As the regularisation parameter increases the bias increases and the variance decreases. This happens because as the model becomes more restricted and simpler, sacrificing some flexibility to reduce the variance and improve generalization, the bias increases. Similarly this causes the variance to reduce before tending to 0 as the regularization parameter increases. Thus, the Mean Squared Error is caused solely by the bias squared.

There is also a plot for the coefficients of the Lasso Model vs the regularization parameter (lambda)
```{r}
...
For Lasso Regression the coefficients of the Lasso Model stay constant for the initial values of the regularization parameter lambda. However they tend to 0 for log10(lambda) = 1. This means that the coefficients get removed from the model for that particular value of lambda hence creating sparsity.
