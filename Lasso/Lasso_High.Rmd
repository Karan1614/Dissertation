...
There is a sharp increase in the train error before tending to 1980.023 as the regularization parameter increases. The test error decreases gradually before slightly increasing and then decreases again as the regularization parameter increases before tending to a value 1987.873 at log10(lambda) = 1.5. Both the test and train error reach this maximum at this point.
...
For Lasso Regression the coefficients of the Lasso Model stay constant for the initial values of the regularization parameter lambda. However they tend to 0 for log10(lambda) = 1.2 (approx). This means that the coefficients get removed from the model for that particular value of lambda hence creating sparsity.
