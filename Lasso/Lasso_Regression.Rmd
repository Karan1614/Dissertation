...
In this case both the train and test MSE behave in quite a similar manner. There is a sharp increase in the error before tending to 41.25470  for the test error and 41.36407 for the train error as the regularization parameter increases. The train and the test error increase as the bias squared increases as the regulation parameter increases while the variance tends to 0. Thus after a point (in this case log10(lambda) = 1) the mean squared error is just the bias squared. This is seen in the next plot.
...
For Lasso Regression the coefficients of the Lasso Model stay constant for the initial values of the regularization parameter lambda. However they tend to 0 for different values of log10(lambda). This means that the coefficients get removed from the model for that particular value of lambda hence creating sparsity.
